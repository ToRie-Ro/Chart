    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        inputDecorationTheme: InputDecorationTheme(
          ...
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(color: blue)),
        ),
      ),
...
class Chat {
  const Chat(this.name, this.message, this.time, this.color, {this.unread = 0, this.online = false});
  final String name;
...
const chats = [
  Chat('SabayChat Team', 'Welcome to SabayChat!', '09:42', blue, unread: 2, online: true),
  Chat('Dara Sok', 'See you tomorrow 👋', 'Yesterday', Color(0xFF8B5CF6), online: true),
  Chat('Design Community', 'New design files are ready', 'Mon', Color(0xFFFF9F43), unread: 8),
...
class _HomeShellState extends State<HomeShell> {
  ...
  Widget build(BuildContext context) {
    final pages = [
      ChatListPage(search: search, filter: filter, onSearch: (v) => setState(() => search = v), onFilter: (v) => setState(() => filter = v)),
      const Center(child: EmptyFeature(icon: Icons.call_outlined, title: 'Calls', detail: 'Your secure calls will appear here.')),
      const SettingsPage(),
    ];
    return Scaffold(
      ...
      floatingActionButton: tab == 0
          ? FloatingActionButton(onPressed: () => _showNewChat(context), backgroundColor: blue, child: const Icon(Icons.add))
          : null,
    );
  }
    ...
    showModalBottomSheet<void>(
      ...
      builder: (_) => const Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.person_add_alt_1, color: blue, size: 36),
          SizedBox(height: 12),
          Text('Start a new chat', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          SizedBox(height: 8),
          Text('Search by username, email, user ID, QR code, or invite link.', textAlign: TextAlign.center, style: TextStyle(color: muted)),
          SizedBox(height: 20),
          TextField(decoration: InputDecoration(hintText: 'Username or user ID', prefixIcon: Icon(Icons.search))),
        ]),
      ),
    );
  }
...
class ChatListPage extends StatelessWidget {
  const ChatListPage({required this.search, required this.filter, required this.onSearch, required this.onFilter, super.key});
  final String search;
  final String filter;
  ...
  Widget build(BuildContext context) {
    final visible = chats.where((chat) {
      ...
      final matchesFilter = filter == 'All' || (filter == 'Groups' ? chat.name.contains('Community') || chat.name.contains('News') : !chat.name.contains('Community') && !chat.name.contains('News'));
        ...
        SliverPadding(
          ...
          sliver: SliverToBoxAdapter(child: Row(children: [
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Good to see you', style: TextStyle(color: muted, fontSize: 12)),
              SizedBox(height: 2),
              Text('SabayChat', style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold)),
            ])),
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7), decoration: BoxDecoration(color: surface, borderRadius: BorderRadius.circular(30)), child: const Row(children: [Icon(Icons.circle, size: 8, color: green), SizedBox(width: 6), Text('Live', style: TextStyle(color: muted, fontSize: 12))])),
...
class ChatRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18), side: const BorderSide(color: border, width: .5)),
      child: InkWell(
        ...
        child: Padding(padding: const EdgeInsets.all(13), child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(chat.name, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 16)), const SizedBox(height: 5), Text(chat.message, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: muted))])),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(chat.time, style: const TextStyle(color: muted, fontSize: 11)), if (chat.unread > 0) Container(margin: const EdgeInsets.only(top: 8), padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: const BoxDecoration(color: blue, shape: BoxShape.circle), child: Text('${chat.unread}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)))]),
        ])),
      ),
    );
  }
...
class SettingsPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 30), children: [
    const Text('Settings', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
    const SizedBox(height: 20),
    Card(color: surface, child: ListTile(leading: const CircleAvatar(backgroundColor: blue, child: Icon(Icons.person)), title: const Text('Your profile'), subtitle: const Text('Manage your account and Premium status', style: TextStyle(color: muted)), trailing: const Icon(Icons.chevron_right), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ProfilePage())))),
    const SizedBox(height: 18),
    const Text('SABAYCHAT', style: TextStyle(color: muted, fontSize: 12, fontWeight: FontWeight.bold)),
    const SizedBox(height: 8),
    SettingsTile(icon: Icons.lock_outline, title: 'Privacy & Security', detail: 'Password and device access', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PrivacyPage()))),
    SettingsTile(icon: Icons.devices_outlined, title: 'Devices & Sessions', detail: 'Manage signed-in devices', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DevicesPage()))),
...
class SettingsTile extends StatelessWidget {
  const SettingsTile({required this.icon, required this.title, required this.detail, required this.onTap, super.key});
  final IconData icon;
  final String title;
  final String detail;
  final VoidCallback onTap;
  @override
  Widget build(BuildContext context) => Card(color: surface, margin: const EdgeInsets.only(bottom: 8), child: ListTile(leading: Icon(icon, color: blue), title: Text(title), subtitle: Text(detail, style: const TextStyle(color: muted, fontSize: 12)), trailing: const Icon(Icons.chevron_right, color: muted), onTap: onTap));
}
...
class InfoCard extends StatelessWidget {
  ...
  final String value;
  @override
  Widget build(BuildContext context) => Card(color: surface, child: ListTile(title: Text(label, style: const TextStyle(color: muted, fontSize: 12)), subtitle: Text(value, style: const TextStyle(color: Colors.white, fontSize: 16))));
}
...
class _PrivacyPageState extends State<PrivacyPage> {
  bool twoStep = false;
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Privacy & Security')), body: ListView(padding: const EdgeInsets.all(20), children: [
    Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF123C8C), surface]), borderRadius: BorderRadius.circular(20)), child: const Row(children: [SizedBox(width: 72, height: 72, child: CircularProgressIndicator(value: .95, strokeWidth: 8, color: green, backgroundColor: surface)), SizedBox(width: 16), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Privacy Score', style: TextStyle(color: muted)), Text('95  Excellent', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), Text('Your data is well protected.', style: TextStyle(color: muted, fontSize: 12))])])),
    const SizedBox(height: 22), const Text('SECURITY', style: TextStyle(color: muted, fontSize: 12, fontWeight: FontWeight.bold)),
...
class DevicesPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Devices & Sessions')), body: ListView(padding: const EdgeInsets.all(20), children: [
...
class EmptyFeature extends StatelessWidget {
  const EmptyFeature({required this.icon, required this.title, required this.detail, super.key});
  final IconData icon;
  final String title;
  final String detail;
  @override
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 52, color: blue), const SizedBox(height: 16), Text(title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 6), Text(detail, style: const TextStyle(color: muted))]));
}
